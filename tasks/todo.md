...
**Status: AWAITING ERIC'S APPROVAL — no app code has been written.**
...
- [ ] `git init`, set your git identity (ericmiguelsoto@gmail.com)
- [ ] Folder structure: `docs/` (app), `tests/`, `tools/`, `.claude/hooks/`
- [ ] Copy `block-destructive.sh` hook from tech-advance-app (blocks
      dangerous commands before they run); write a small stop-hook that runs
      `node --test` so I can't claim "done" with failing tests
- [ ] Placeholder `docs/index.html` ("Budget — under construction")
- [ ] Create public GitHub repo `simple-budget` with `gh`, push, turn on
      Pages (main branch, `/docs` folder)
- [ ] **Your check:** `https://ericmiguelsoto.github.io/simple-budget/`
      loads on your phone. Bookmark it.

### Phase 1 — Money and month logic, tests first
- [ ] Write failing tests: dollars→cents parsing ("12", "12.3", "$1,234.56",
...
- [ ] Run `node --test` — confirm they FAIL (nothing exists yet; proves the
      tests are real)
- [ ] Build `docs/js/logic.js` (pure calculation functions) until all pass
- [ ] Build `docs/js/storage.js` (load/save/seed localStorage; seeds
      categories **Groceries** and **Random** on first run)
- [ ] Commit. **Your check:** none — I'll show you the passing test output.

### Phase 2 — Home screen + quick entry (the core loop)
- [ ] `docs/index.html` — real structure: three views, bottom tabs, + button
- [ ] `docs/css/styles.css` — dark mobile-first layout
- [ ] `docs/js/app.js` — render Home from storage; add-expense sheet with
      auto-focused amount field (numeric keypad pops immediately), category
      chips, today-by-default date, optional note
- [ ] Deploy. **Your check:** add a real expense start-to-finish in under
...
- [ ] Settings view: single monthly-budget field (typed as dollars, stored
      as cents); add / rename / archive categories
- [ ] Home: "This month" expense list; tap an entry → edit or delete
- [ ] Deploy. **Your check:** set your real monthly budget; Home now shows
      "left of $X" correctly; fat-finger an expense, fix it by tapping it.

### Phase 4 — Make it installable (PWA)
- [ ] `manifest.webmanifest` — app name, colors, standalone display
- [ ] App icons generated by a small Node script in `tools/` (geometric
      mark, no downloads, no emoji)
- [ ] `docs/sw.js` — network-first service worker, versioned cache, offline
...
- [ ] Deploy. **Your check (on the S25 Ultra):** install FROM THE REAL URL
...
- [ ] History view: month rows, tap to expand category breakdown
- [ ] Settings: Export backup (downloads a JSON file) / Import (restores it)
- [ ] Deploy. **Your check:** history shows August; export a backup, delete
      an expense, import — it's back.

### Phase 6 — Polish + wrap-up
- [ ] Empty states (no expenses yet, no targets yet), over-budget styling,
      spacing/typography pass
- [ ] Update CLAUDE.md with final structure; write the review section below;
      final lessons into tasks/lessons.md
- [ ] Deploy. **Your check:** daily-driver sign-off.
...
(Filled in at the end: what was built, what changed from the plan, and why.)
